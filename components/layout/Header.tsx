    ...
    >
      <div className="relative mx-auto grid max-w-7xl grid-cols-[auto_minmax(0,1fr)] items-center gap-x-1.5 px-2.5 py-2.5 min-[360px]:gap-x-2 min-[360px]:px-3 md:grid-cols-[auto_1fr_auto] md:items-center md:gap-x-8 md:px-10 md:py-5 lg:gap-x-10 lg:px-12 xl:px-14">
        ...
        <button
          ...
          className="relative z-10 ml-auto inline-flex h-11 w-11 items-center justify-center rounded-full border border-white/15 bg-black/60 text-white transition-colors hover:border-white/30 hover:bg-black/80 md:hidden"
        >
      ...
      <AnimatePresence>
        {menuOpen && (
          <>
            ...
            />
            <motion.aside
              className="fixed right-0 top-0 z-[60] h-dvh w-[min(86vw,360px)] border-l border-white/10 bg-black/95 px-6 pb-8 pt-24 backdrop-blur-2xl md:hidden"
              initial={{ x: "100%" }}
              animate={{ x: 0 }}
              exit={{ x: "100%" }}
            ...
            >
              <nav className="flex flex-col gap-4">
                {NAV_LINKS.map((link) => {
                  ...
                  const itemClass =
                    "rounded-xl border border-white/10 bg-white/[0.02] px-4 py-3 text-sm font-semibold uppercase tracking-[0.14em] text-white/85 transition-colors hover:border-white/20 hover:bg-white/[0.06]";
                ...
                <a
                  href={contactHref}
                  className="mt-2 inline-flex items-center justify-center rounded-full border border-white/20 bg-white/[0.06] px-5 py-3 text-xs font-semibold uppercase tracking-[0.14em] text-white transition-colors hover:border-white/35 hover:bg-white/[0.1]"
                  onClick={() => setMenuOpen(false)}
                >
                  Proje Başlat
                </a>
